...
if mktOpen <= now_local_tz <= mktClose:  # only run during market hours
    ...
    data = (
        web.DataReader(symbols, "iex-tops")
        .assign(
            lastSaleTime=lambda df: pd.to_datetime(df.lastSaleTime, unit="ms")
        )
        .assign(
            lastUpdated=lambda df: pd.to_datetime(df.lastUpdated, unit="ms")
        )
