...
def split_timestamp(df, timestamp):
    ...
    df = df.assign(
        queryTime=lambda df: timestamp,
        year=lambda df: timestamp.year,  # make year
        month=lambda df: timestamp.month,  # make month
        day=lambda df: timestamp.day,  # make day
        time=lambda df: timestamp.strftime("%H:%M:%S"),
    )  # make time
    return df


def write_to_parquet(
    df, root_path, partition_cols=["year", "month", "day", "time"], logger=None
):
    """
    ...
    """
    if not logger:
        raise ValueError("must use logger object")
    try:
